...
func TestSignerRequest(t *testing.T) {
	testFn := func(t *testing.T, test httpsigTest) {
		s, a, err := ConfigureSigner(test.prefs, test.digestAlg, test.headers, test.scheme, 0)
		if err != nil {
			t.Fatalf("%s", err)
		}
...
func TestSignerResponse(t *testing.T) {
	testFn := func(t *testing.T, test httpsigTest) {
		s, _, err := ConfigureSigner(test.prefs, test.digestAlg, test.headers, test.scheme, 0)
...
func Test_Signing_HTTP_Messages_AppendixC(t *testing.T) {
	...
	for _, test := range specTests {
		t.Run(test.name, func(t *testing.T) {
			...
			s, _, err := ConfigureSigner([]Algorithm{RSA_SHA256}, DigestSha256, test.headers, Authorization, 0)
			if err != nil {
...
func TestSigningEd25519(t *testing.T) {
	...
	for _, test := range specTests {
		t.Run(test.name, func(t *testing.T) {
			...
			s, _, err := ConfigureSigner([]Algorithm{ED25519}, DigestSha256, test.headers, Authorization, 0)
			if err != nil {
				t.Fatalf("error creating signer: %s", err)
			}
